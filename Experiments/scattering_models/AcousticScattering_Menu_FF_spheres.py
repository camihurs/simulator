...
from scipy.special import eval_legendre, jvp, spherical_jn, spherical_yn, yvp
...
def MainMenu():
    ...
    print(
        "1. FORM FUNCTIONS FOR SPHERICAL SHAPES: Generate and plot Form Functions for different spherical shapes."
    )
    print(
        "3. ECHOES FROM SPHERICAL SHAPES: Generate synthetic echoes for different spherical shapes and save the results."
    )
...
def menuReferenceFormFunEchoes():
    ...
    print(
        "6. Echo from a fluid-filled spherical shell in free field using a Ricker incident signal."
    )
    print(
        "7. Echo from a fluid-filled spherical shell in free field using a Chirp incident signal."
    )
    print("8. Exit.")


def SaveData(
    filename, echo_normalized, sampling_rate, source, field, params, target, condition
):
    ...
    if choice == "1":
        ...
        output_path = os.path.abspath(
            os.path.join(r"C:\Users\camih\Documents\4. Python\3. OCEANS 2026", filename)
        )
...
def SetReferenceParameters(
 ...
 function (Press Enter to use default value = {default_freq_points}): "
    ...
    user_input = input(
        f"Enter the number of modes you want to compute for the form function (Press\
 Enter to use default value = {default_max_modes}): "
    )
    max_modes = int(user_input) if user_input.strip() else default_max_modes

    user_input = input(
        f"Enter the maximum ka you want to compute for the form function (Press Enter\
 to use default value = {default_xmax}): "
    )
    xmax = int(user_input) if user_input.strip() else default_xmax

    user_input = input(
        f"Enter the radius of the spherical target (Press Enter to use default value\
 = {default_a}): "
    )
...
def CheckRigidSphereFormFunction():
 ...
 (near and far field)."
    ...
    print(
        "The paper does not specify the speed of sound in water neither the radius of the sphere.\
...
def CheckSolidSphereFormFunction():
 ...
 (near and far field)."
    ...
    print(
        "The paper presents results for several materials. We will show the one corresponding to\
...
def CheckEchoRigidSine():
 ...
 immersed in a fluid. The Journal of the Acoustical Society of America, 45(4), 900-910."
    ...
    print(
        "The paper uses the far field expression of the form function. We use the general expression."
    )
 ...
 Since it is a rigid sphere, its material properties are not needed."
    ...
    print(
        "The document does not mention either the distance between the sonar and the target,\
 ...
 a different result from the reference paper."
    ...
    rayleigh_distance, field = RayleighDistance(
        params["a"], f0, params["c1"], params["r"]
    )
...
def CheckEchoFluidRicker():
 ...
 sediments. Applied Acoustics, 69(5), 422-437."
    ...
    print(
        "The paper uses the general expression (near and far field) of the form function as we do."
    )
...
def CheckEchoFluidChirp():
 ...
 a different result from the reference paper."
    ...
    f_start, f_end, duration, sampling_rate, Hanning, MintoMax = (
        ChirpDefaultParametersReference()
    )
    source_signal, t, f0 = chirp_source(
        f_start, f_end, duration, sampling_rate, MintoMax, Hanning
    )
...
def ComputeFormFunctionRigid(params, check=False):
    ...
    if not check:
        fmin, fmax, frequency_points, xmin, xmax = GetFreqsFormfunction(
            params["fmin"],
            params["fmax"],
    ...
    if check:
        print(
            "Checking the form function for the rigid sphere in free field according to the reference..."
        )
        ...
        f_farfield = (
            (-2 * f) / (ka)
        )  # Da el mismo resultado que el anterior, pero sólo en magnitud. En fase sí cambia un poco.
        ...
        PlotFormFunction(
            f_farfield,
            ka,
            "rigid sphere (according to the reference paper)",
            "free field - far field",
            params,
            fmin,
            fmax,
            frequency_points,
        ...
        print(
            "The FREQUENCY STEP to get the form function is: ",
            (fmax - fmin) / frequency_points,
            "Hz",
        )
        choice = input(
 ...
 above the seabed? (1: Free field, 2: Above the seabed): "
        ...
        if choice == "1":
            ...
            print(
                f"Computing form function for the rigid sphere above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            )
            gamma_11 = reflec_coef(
                theta_i, np.abs(freqs), params["rho1"], params["rho4"], c1, params["c4"]
            )
            gamma_11_squared = np.power(gamma_11, 2)

            print(
                f"Computing the monostatic scattering form function for the rigid sphere above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            )
            ...
            print(
                f"Computing the bistatic scattering form function for the rigid sphere above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            )
            ...
            formfunction = GetFormFunctionAbove(
                f1, f2, gamma_11, gamma_11_squared, k, a, theta_i
            )
...
def ComputeFormFunctionSolid(params, check=False):
    ...
    )  # La cantidad de puntos podría ser un parámetro a variar
    ...
    if check:
        ...
        for n in tqdm(range(n_max_solid), total=n_max_solid, desc="Modes"):
            fn = ModesSolid(
                n, k, freqs, x, x1, x2, np.pi, 0, rho1, rho2, r, "far-field"
            )
            f += fn
        ...
        PlotFormFunction(
            f_farfield,
            ka,
            "solid " + material_sphere + " sphere (according to the reference paper)",
            "free field - far field",
            params,
            fmin,
            fmax,
 ...
 in free field or above the seabed? (1: Free field, 2: Above the seabed): "
        ...
        if choice == "1":
            print(
                f"Computing form function for the ({material_sphere}) solid sphere in free field..."
            )
            ...
            print(
                f"Computing form function for the ({material_sphere}) solid sphere above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            )
            gamma_11 = reflec_coef(
                theta_i, np.abs(freqs), rho1, params["rho4"], c1, params["c4"]
            )
            gamma_11_squared = np.power(gamma_11, 2)

            print(
                f"Computing the monostatic scattering form function for the solid ({material_sphere}) elastic sphere above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            )
            f1 = np.zeros(freqs.size, dtype=complex)
            for n in tqdm(range(n_max_solid), total=n_max_solid, desc="Modes"):
                fn = ModesSolid(n, k, freqs, x, x1, x2, np.pi, 0, rho1, rho2, r)
            ...
            print(
                f"Computing the bistatic scattering form function for the solid ({material_sphere}) elastic sphere above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            )
            f2 = np.zeros(freqs.size, dtype=complex)
            for n in tqdm(range(n_max_solid), total=n_max_solid, desc="Modes"):
...
def ComputeFormFunctionVacuum(params, check=False):
 ...
 in free field according to the reference..."
        ...
        for n in tqdm(range(n_max_vacuum), total=n_max_vacuum, desc="Modes"):
            fn = ModesVacuum(
                n, k1, freqs, x, xs, xL, ys, yL, np.pi, 0, rho_tilde, r, "far-field"
            )
            f += fn
        ...
        PlotFormFunction(
            f,
            ka,
            f"Vacuum-filled {material_sphere} spherical shell (according to the reference paper)",
            "free field - far field",
            params,
            fmin,
            fmax,
 ...
 spherical shell in free field or above the seabed? (1: Free field, 2: Above the seabed): "
        ...
        if choice == "1":
            print(
                f"Computing form function for the vacuum-filled ({material_sphere}) spherical shell in free field..."
            )
            ...
            for n in tqdm(range(n_max_vacuum), total=n_max_vacuum, desc="Modes"):
                fn = ModesVacuum(
                    n, k1, freqs, x, xs, xL, ys, yL, np.pi, 0, rho_tilde, r
                )
                f += fn
            ...
            PlotFormFunction(
                f,
                ka,
                f"Vacuum-filled {material_sphere} spherical shell",
                "free field",
                params,
                fmin,
                fmax,
            ...
            print(
                f"Computing form function for the vacuum-filled {material_sphere} spherical shell above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            )
            gamma_11 = reflec_coef(
                theta_i, np.abs(freqs), params["rho1"], params["rho4"], c1, params["c4"]
            )
            ...
            print(
                f"Computing the monostatic scattering form function for the vacuum-filled {material_sphere} spherical shell above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            )
            f1 = np.zeros(freqs.size, dtype=complex)
            for n in tqdm(range(n_max_vacuum), total=n_max_vacuum, desc="Modes"):
                fn = ModesVacuum(
            ...
            print(
                f"Computing the bistatic scattering form function for the vacuum-filled {material_sphere} spherical shell above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            )
            f2 = np.zeros(freqs.size, dtype=complex)
            for n in tqdm(range(n_max_vacuum), total=n_max_vacuum, desc="Modes"):
                fn = ModesVacuum(
                    n, k1, freqs, x, xs, xL, ys, yL, theta_i, 1, rho_tilde, r
                )
                f2 += fn

            formfunction = GetFormFunctionAbove(
                f1, f2, gamma_11, gamma_11_squared, k1, a, theta_i
            )
...
def ComputeFormFunctionFluid(params, check=False):
 ...
 shell in free field according to the reference..."
        ...
        # Cálculo de los modos
        for n in tqdm(
            range(n_max_fluid_filled), total=n_max_fluid_filled, desc="Modes"
        ):
            fn = ModesFluid(
                n,
                k1,
        ...
        PlotFormFunction(
            f,
            ka,
            f"{inner_material}-filled {material_sphere} spherical shell (according to the reference paper)",
            "free field - far field",
            params,
            fmin,
            fmax,
 ...
 {material_sphere} spherical shell in free field or above the seabed? (1: Free field, 2: Above the seabed): "
        ...
        if choice == "1":
            print(
                f"Computing form function for the {inner_material}-filled {material_sphere}\
 spherical shell in free field..."
            ...
            for n in tqdm(
                ...
                fn = ModesFluid(
                    n, k1, k_d2, k_s2, k3, freqs, np.pi, 0, rho1, rho2, rho3, a, b, r
                )
                f += fn
            ...
            PlotFormFunction(
                f,
                ka,
                f"{inner_material}-filled {material_sphere} spherical shell",
                "free field",
                params,
                fmin,
                fmax,
 ...
 spherical shell above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            ...
            print(
                f"Computing the monostatic scattering form function for the {inner_material}-filled \
 {material_sphere} spherical shell above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            ...
            print(
                f"Computing the bistatic scattering form function for the {inner_material}-filled\
 {material_sphere} spherical shell above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            )
            f2 = np.zeros(freqs.size, dtype=complex)
            for n in tqdm(
                ...
                fn = ModesFluid(
                    n, k1, k_d2, k_s2, k3, freqs, theta_i, 1, rho1, rho2, rho3, a, b, r
                )
                f2 += fn
...
def GenerateEchoesSpherical():
    ...
    if choice == "1":
        ...
        PrintSummarySourceEcho(
            source, duration, sampling_rate, params["fft_points"], f0
        )
    else:
        f0, duration, sampling_rate = SinusoidDefaultParameters()
        source_signal, t = sinusoid_source(f0, duration, sampling_rate)
    ...
    while True:
        ...
        if option == "1":
            GenerateEchoesRigid(
                S_f, freqs_positive, sampling_rate, source, field, params
            )
        elif option == "2":
            GenerateEchoesSolid(
                S_f, freqs_positive, sampling_rate, source, field, params
...
def GenerateEchoesRigid(
    S_f, freqs_positive, sampling_rate, source, field, params, check=False
):
    ...
    if check:
        print(
            "Computing the echo for the rigid sphere in free field according to the reference..."
        )
        n_max_rigid = params["n_max"]
        ...
        PlotFormFunction(
            ...
            ka,
            "rigid sphere",
            "free field (computed to get the reference echo)",
            params,
            0,
            np.max(freqs_positive),
            len(freqs_positive),
            0,
            np.max(ka),
        )
        ...
        echo_rigid, echo_rigid_normalized = GetEchoFF(
            S_f, f_final_rigid, fft_points, target
        )

        PlotEcho(
            echo_rigid_normalized,
            sampling_rate,
            source,
            field,
            params,
        ...
        params["n_max"] = n_max_rigid
        choice = input(
            "Do you want to compute the echo for the rigid sphere in free field or above the seabed?\
 (1: Free field, 2: Above the seabed): "
        ...
        if choice == "1":
            ...
            PlotFormFunction(
                ...
                ka,
                "rigid sphere",
                "free field (computed to get the echo)",
                params,
                0,
                np.max(freqs_positive),
                len(freqs_positive),
...
mSlant_{field}_{fft_points}FFT_NoTrunc.h5"
            ...
            print(
                f"Computing the echo for the rigid sphere above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            )
            target = "RigidSphere"
            ...
            echo_rigid_above_normalized, f_above = GetEchoAbove(
                f1,
                f2,
                gamma_11,
                k,
            ...
            PlotFormFunction(
                ...
                ka,
                "rigid sphere",
                f"Above the seabed (computed to get the echo) {int(np.rad2deg(theta_i))}º grazing angle",
                params,
                0,
                np.max(freqs_positive),
                len(freqs_positive),
...
def GenerateEchoesSolid(S_f, freqs_positive, sampling_rate, source, field, params):
    ...
    choice = input(
        f"Do you want to compute the echo for the {material_sphere} solid sphere in free\
 field or above the seabed? (1: Free field, 2: Above the seabed): "
    )
    if choice == "1":
        print(
            f"Computing the echo for the {material_sphere} solid sphere in free field..."
        )
        ...
        echo_solid, echo_solid_normalized = GetEchoFF(
            S_f, f_final_solid, fft_points, target
        )

        PlotEcho(
            echo_solid_normalized,
            sampling_rate,
            source,
            field,
            params,
...
mSlant_{field}_{fft_points}FFT_NoTrunc.h5"
        ...
        print(
            f"Computing the echo for the {params['material_sphere']} solid sphere above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
        )
        target = "Sol" + f"{params['material_sphere']}" + "Sphere"
        condition = f"Above({int(np.rad2deg(theta_i))}º)"

        gamma_11 = reflec_coef(
            theta_i, freqs_positive, params["rho1"], params["rho4"], c1, params["c4"]
        )
        gamma_11_squared = np.power(gamma_11, 2)

        print(
            f"Computing the monostatic scattering form function for the {material_sphere} solid sphere above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
        )
        f1 = np.zeros(freqs_positive.size, dtype=complex)
        for n in tqdm(range(n_max_solid), total=n_max_solid, desc="Modes"):
            fn = ModesSolid(
        ...
        print(
            f"Computing the bistatic scattering form function for the {material_sphere} solid sphere above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
        )
        f2 = np.zeros(freqs_positive.size, dtype=complex)
        for n in tqdm(range(n_max_solid), total=n_max_solid, desc="Modes"):
            fn = ModesSolid(
        ...
        echo_solid_above_normalized, f_above = GetEchoAbove(
            f1, f2, gamma_11, k, a, theta_i, gamma_11_squared, S_f, fft_points, "solid"
        )
...
def GenerateEchoesVacuum(S_f, freqs_positive, sampling_rate, source, field, params):
    ...
    choice = input(
        f"Do you want to compute the echo for the vacuum-filled {material_sphere} spherical\
 shell in free field or above the seabed? (1: Free field, 2: Above the seabed): "
    )

    if choice == "1":
        print(
            f"Computing the echo for the vacuum-filled {material_sphere} spherical shell in free field..."
        )
        ...
        echo_vacuum, echo_vacuum_normalized = GetEchoFF(
            S_f, f_final_vacuum, params["fft_points"], target
        )

        PlotEcho(
            echo_vacuum_normalized,
            sampling_rate,
            source,
            field,
            params,
...
mSlant_{field}_{params['fft_points']}FFT_NoTrunc.h5"
        ...
        print(
            f"Computing the echo for the vacuum-filled {material_sphere} spherical shell above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
        )
        ...
        print(
            f"Computing the monostatic scattering form function for the vacuum {material_sphere} spherical shell above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
        )
        f1 = np.zeros(freqs_positive.size, dtype=complex)
        for n in tqdm(range(n_max_vacuum), total=n_max_vacuum, desc="Modes"):
            fn = ModesVacuum(
        ...
        print(
            f"Computing the bistatic scattering form function for the vacuum {material_sphere} spherical shell above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
        )
        f2 = np.zeros(freqs_positive.size, dtype=complex)
        for n in tqdm(range(n_max_vacuum), total=n_max_vacuum, desc="Modes"):
            fn = ModesVacuum(
        ...
        echo_vacuum_above_normalized, f_above = GetEchoAbove(
            f1,
            f2,
            gamma_11,
            k1,
...
def GenerateEchoesFluid(
    ...
    if check:
        print(
            f"Computing the echo for the {inner_material}-filled {material_sphere} spherical shell in free field according to the reference..."
        )
        n_max_fluid_filled = params["n_max"]
        ...
        echo_fluid_filled, echo_fluid_filled_normalized = GetEchoFF(
            S_f, f_final_fluid_filled, fft_points, target
        )

        PlotEcho(
            echo_fluid_filled_normalized,
            sampling_rate,
            source,
            field,
            params,
        ...
        params["n_max"] = n_max_fluid_filled
        choice = input(
            f"Do you want to compute the echo for the {inner_material}-filled {material_sphere}\
 spherical shell in free field or above the seabed? (1: Free field, 2: Above the seabed): "
        )

        if choice == "1":
            print(
                f"Computing the echo for the {inner_material}-filled {material_sphere} spherical shell in free-field..."
            )
            ...
            f_final_fluid_filled = GetFormFunctionEcho(f)
            echo_fluid_filled, echo_fluid_filled_normalized = GetEchoFF(
                S_f, f_final_fluid_filled, params["fft_points"], target
            )
...
mSlant_{field}_{params['fft_points']}FFT_NoTrunc.h5"
            ...
            print(
                f"Computing the echo for the {inner_material}-filled {material_sphere} spherical shell above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            )
            ...
            gamma_11 = reflec_coef(
                theta_i, np.abs(freqs_positive), rho1, params["rho4"], c1, params["c4"]
            )
            gamma_11_squared = np.power(gamma_11, 2)

            print(
                f"Computing the monostatic scattering form function for the {inner_material}-filled {material_sphere} spherical shell above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            )
            f1 = np.zeros(freqs_positive.size, dtype=complex)
            for n in tqdm(
                range(n_max_fluid_filled), total=n_max_fluid_filled, desc="Modes"
            ...
            print(
                f"Computing the bistatic scattering form function for the {inner_material}-filled {material_sphere} spherical shell above the seabed with {int(np.rad2deg(theta_i))}º grazing angle..."
            )
            f2 = np.zeros(freqs_positive.size, dtype=complex)
            for n in tqdm(
                range(n_max_fluid_filled), total=n_max_fluid_filled, desc="Modes"
            ...
            echo_fluid_filled_above_normalized, f_above = GetEchoAbove(
                f1,
                f2,
                gamma_11,
                k1,
            ...
            PlotEcho(
                echo_fluid_filled_above_normalized,
                sampling_rate,
                source,
                field,
                params,
...
def PlotEcho(
    ...
    if choice == "1":
        ...
        plt.title(
            f"Acoustic backscattering of a {target} (radius: {params['a']:.3f} m) in {condition}",
            fontsize=18,
        )
        plt.tick_params(axis="both", which="major", labelsize=18)
        plt.grid()
        plt.axvline(
            x=(params["r"] - params["a"]) * 1000 / params["c1"],
            color="red",
            linestyle="--",
            label=f"Specular echo = {specular_echo:5f} s",
        )
        plt.legend()
        plt.show()
        plt.clf()
...
def PlotSource(
    ...
    if choice == "1":
        ...
        plt.plot(
            t * 1000, source_signal
        )  # Convertimos el tiempo a milisegundos para que coincida con el gráfico
        ...
        plt.plot(
            filtered_freqs / 1000,
            spectrum_dB,
            label=f"Positive FFT (up to {cutoff_freq} Hz)",
        )
...
def PlotEchoinFreq(echo, params, check=False):
    ...
    if choice == "1":
        ...
        plt.title(
            f"FFT Spectrum of the echo from a {params['target']} (radius: {params['a']:.3f} m) in {params['condition']}",
            fontsize=18,
        )
        plt.xlabel("Frequency (Hz)")
        plt.ylabel("Amplitude")
...
def PlotFormFunction(
    f, ka, target, condition, params, fmin, fmax, frequency_points, xmin, xmax
):
    print(
        "\n..............................................................................."
    )
    choice = input(
        f"Do you want to plot the form function for the {target} in {condition}? (1: Yes, 2: No): "
    )
    if choice == "1":
        ...
        plt.title(
            f"Form function $(F_{{\\infty}})$ of a {target}\n (radius: {params['a']:.3f} m) in {condition}",
            fontsize=18,
        )
        plt.grid()
        plt.show()
        ...
        if any(word in target for word in ["rigid", "solid", "vacuum"]):
            plt.title(
                f"Backscattered target strength of a {target}\n (radius: {params['a']:.3f} m) in {condition}",
                fontsize=18,
            )
        else:
            plt.title(
                f"Backscattered target strength of a {target}\n ((radius: {params['a']} m, thickness (h) =\
 {params['h'] * 100:.1f} %) in {condition}",
                fontsize=18,
            )
        plt.tick_params(axis="both", which="major", labelsize=18)
        ...
        if any(word in target for word in ["rigid", "solid", "vacuum"]):
            plt.title(
                f"Phase of the form function of a {target}\n (radius: {params['a']:.3f} m) in {condition}",
                fontsize=18,
            )
        else:
            plt.title(
                f"Phase of the form function of a {target}\n (radius: {params['a']} m, thickness (h) = {params['h'] * 100:.1f} %) in {condition}",
                fontsize=18,
            )
        plt.tick_params(axis="both", which="major", labelsize=18)
        plt.xlim(xmin, xmax)
        ...
        print(
            f"You chose not to compute the form function for the {target} sphere in {condition}."
        )
...
def GetParameters():
    ...
    (
        ...
        default_c4,
        default_delta,
    ) = DefaultParametersMedia()

    default_rho_tilde = default_rho1 / default_rho2

    # 1. Parameters of the sonar and target--------------------------------------
    (
        default_sonar_position,
        default_target_position,
        default_r,
        default_theta_i,
        default_fft_points,
    ) = DefaultParametersSonarTarget()
    ...
    (
        ...
        default_fmin_rigid,
        default_fmax_rigid,
        default_frequency_points_rigid,
        default_xmin_solid,
        default_xmax_solid,
        default_fmin_solid,
        default_fmax_solid,
        default_frequency_points_solid,
        default_xmin_vacuum,
        default_xmax_vacuum,
        default_fmin_vacuum,
        default_fmax_vacuum,
        default_frequency_points_vacuum,
        default_xmin_fluid_filled,
        default_xmax_fluid_filled,
        default_fmin_fluid_filled,
        default_fmax_fluid_filled,
        default_frequency_points_fluid_filled,
        default_n_max_fluid_filled,
        default_n_max_rigid,
        default_n_max_solid,
        default_n_max_vacuum,
    ) = GetDefaultFormFuctionParameters(default_c1, default_a)
 ...
 and form functions)? (1: Yes, 2: No): "
    ...
    if choice == "1":
        ...
        theta_i, r = inc_graz_angle(
            sonar_position[0], sonar_position[1], sonar_position[2], target_position
        )
        ...
        (
            ...
            fmin_rigid,
            fmax_rigid,
            frequency_points_rigid,
            xmin_solid,
            xmax_solid,
            fmin_solid,
            fmax_solid,
            frequency_points_solid,
            xmin_vacuum,
            xmax_vacuum,
            fmin_vacuum,
            fmax_vacuum,
            frequency_points_vacuum,
            xmin_fluid_filled,
            xmax_fluid_filled,
            fmin_fluid_filled,
            fmax_fluid_filled,
            frequency_points_fluid_filled,
        ) = SetFormFunctionParameters(
            default_n_max_fluid_filled,
            default_n_max_rigid,
            default_n_max_solid,
            default_n_max_vacuum,
...
def DefaultParametersSonarTarget():
    ...
    default_cross_distance = (
        -9.55
    )  # m (distance in the X direction between the sonar and the target)
    default_y_sonar = 0  # Sonar starts at -1 m in the Y direction
    default_height = 3.6  # m (height of the sonar)
    default_sonar_position = np.array(
        [default_cross_distance, default_y_sonar, default_height]
    )
    ...
    default_theta_i, default_r = inc_graz_angle(
        default_cross_distance, default_y_sonar, default_height, default_target_position
    )
...
def DefaultFormFunctionParametersFluidFilledSphere(default_c1, default_a):
    ...
    default_fmin_fluid_filled = (
        default_xmin_fluid_filled * default_c1 / (2 * np.pi * default_a)
    )
    default_fmax_fluid_filled = (
        default_xmax_fluid_filled * default_c1 / (2 * np.pi * default_a)
    )
...
def printSimulationParameters(
    ...
    print(
        ...
        [sonar_position[0], sonar_position[1], sonar_position[2]],
    )
    ...
    print(
        f"- Incident grazing angle relative to the sediment surface (for the proud case): {np.degrees(theta_i):.2f}º"
    )
 ...
 are used only for the computation of the form functions (not echo)): "
    ...
    print("\nRigid sphere: ")
    print(
        "- Maximum number of modes to be computed for the form function of the rigid sphere: ",
        n_max_rigid,
    )
    ...
    print("\nSolid sphere: ")
    print(
        "- Maximum number of modes to be computed for the form function of the solid sphere: ",
        n_max_solid,
    )
    ...
    print(
        "- Maximum number of modes to be computed for the form function of the vacuum-filled sphere: ",
        n_max_vacuum,
    )
    ...
    print("\nFluid-filled sphere: ")
    print(
        "- Maximum number of modes to be computed for the form function of the fluid-filled sphere: ",
        n_max_fluid_filled,
    )
    ...
    print(
        f"- Thickness of the fluid-filled elastic spherical shell (1 - a/b):  {h * 100:.2f} %"
    )
    print(f"- Material of the spherical shape: {material_sphere}")
    print(
        f"- Density of the material of the spherical shape ({material_sphere}): {rho2} kg/m^3"
    )
    print(
        f"- Speed of compressional waves in the material of the spherical shape ({material_sphere}): {cd2} m/s"
    )
    print(
        f"- Speed of shear waves in the material of the spherical shape ({material_sphere}): {cs2} m/s"
    )

    # -----------------------------------------------------------------------------------
    print("\nMEDIUM 3(inner fluid) properties: ")
    print(
        f"- Inner material of the fluid-filled elastic spherical shell: {inner_material}"
    )
...
def SetMediaParameters(
    ...
    # Medium2
    material_sphere, rho2, cd2, cs2 = SetMedium2(
        default_material_sphere, default_rho2, default_cd2, default_cs2
    )
    rho_tilde = rho1 / rho2

    # Medium3
    inner_material, rho3, c3 = SetMedium3(
        default_inner_material, default_rho3, default_c3
    )

    # Medium4: sedimento
    sediment, rho4, c4, delta = SetMedium4(
...
def SetFormFunctionParameters(
    ...
    # Number of modes
    n_max_fluid_filled, n_max_rigid, n_max_solid, n_max_vacuum = SetMaxModes(
        default_n_max_fluid_filled,
        default_n_max_rigid,
        default_n_max_solid,
        default_n_max_vacuum,
    ...
    xmin_rigid, xmax_rigid, fmin_rigid, fmax_rigid, frequency_points_rigid = (
        SetFormFunctionParametersRigid(
            c1,
            a,
            default_xmin_rigid,
            default_xmax_rigid,
    ...
    xmin_solid, xmax_solid, fmin_solid, fmax_solid, frequency_points_solid = (
        SetFormFunctionParametersSolid(
            c1,
            a,
            default_xmin_solid,
            default_xmax_solid,
    ...
    xmin_vacuum, xmax_vacuum, fmin_vacuum, fmax_vacuum, frequency_points_vacuum = (
        SetFormFunctionParametersVacuum(
            c1,
            a,
            default_xmin_vacuum,
            default_xmax_vacuum,
    ...
    (
        ...
        fmax_fluid_filled,
        frequency_points_fluid_filled,
    ) = SetFormFunctionParametersFluid(
        c1,
        a,
        default_xmin_fluid_filled,
        default_xmax_fluid_filled,
    ...
    if user_input.lower().strip() == "y":
        ...
        while True:
            try:
                ...
                c1 = float(
                    input(
                        "Enter sound speed in medium (m/s) (this also affects the computation\
    ...
    if user_input.lower().strip() == "y":
        ...
        while True:
            try:
                ...
                cd2 = float(
                    input(
                        "Enter compressional wave speed (this also affects the computation\
 of the form functions) (m/s): "
                    )
                )
                cs2 = float(
                    input(
                        "Enter shear wave speed (this also affects the computation of the\
 form functions) (m/s): "
                    )
                )
                break
    ...
    if user_input.lower().strip() == "y":
        ...
        while True:
            try:
                ...
                c3 = float(
                    input(
                        "Enter sound speed (this also affects the computation of the form\
 functions) (m/s): "
                    )
                )
                break
    ...
    if user_input.lower().strip() == "y":
        ...
        while True:
            try:
                ...
                c4 = float(
                    input(
                        "Enter sound speed (this also affects the computation of\
    ...
    if user_input.lower().strip() == "y":
        while True:
            try:
                a = float(
                    input(
                        "Enter outer radius (this also affects the computation of the form\
 functions) (m): "
                    )
                )
                b = float(
                    input(
                        "Enter inner radius (this also affects the computation of the form\
 functions) (m): "
                    )
                )
                if b >= a:
...
def SetFormFunctionParametersRigid(
    c1,
    a,
    default_xmin_rigid,
    default_xmax_rigid,
    ...
    if user_input.lower().strip() == "y":
        while True:
            try:
                ...
                frequency_points_rigid = int(
                    input("Enter the number of frequency points: ")
                )
                break
            except ValueError as e:
...
def SetFormFunctionParametersSolid(
    c1,
    a,
    default_xmin_solid,
    default_xmax_solid,
...
def SetFormFunctionParametersVacuum(
    c1,
    a,
    default_xmin_vacuum,
    default_xmax_vacuum,
...
def SetFormFunctionParametersFluid(
    c1,
    a,
    default_xmin_fluid_filled,
    default_xmax_fluid_filled,
    ...
    if user_input.lower().strip() == "y":
        while True:
            try:
                ...
                frequency_points_fluid_filled = int(
                    input("Enter the number of frequency points: ")
                )
                break
            except ValueError as e:
...
def ChirpDefaultParameters():
    ...
    default_f_end = 10000
    f_end = input(
        f"Enter the end frequency of the chirp (in Hz - Press Enter to use default value = {default_f_end}): "
    )
    f_end = float(f_end) if f_end.strip() else default_f_end

    default_duration = 0.02
    duration = input(
        f"Enter the duration of the chirp (in seconds - Press Enter to use default value = {default_duration}): "
    )
    duration = float(duration) if duration.strip() else default_duration

    default_sampling_rate = 128000
    sampling_rate = input(
        f"Enter the sampling rate (in Hz - Press Enter to use default value = {default_sampling_rate}): "
    )
    sampling_rate = (
        float(sampling_rate) if sampling_rate.strip() else default_sampling_rate
    )
    ...
    choice = input(
        "Do you want the chirp from Min to Max frequency or from Max to Min frequency? (1: Min to Max, 2: Max to Min): "
    )
    if choice == "1":
        MintoMax = True
    else:
...
def RickerDefaultParameters():
    ...
    duration = input(
        f"Enter the duration of the Ricker (in seconds - Press Enter to use default value = {default_duration}): "
    )
    duration = float(duration) if duration.strip() else default_duration

    default_sampling_rate = 128e3
    sampling_rate = input(
        f"Enter the sampling rate of the Ricker (in Hz - Press Enter to use default value = {default_sampling_rate}): "
    )
    sampling_rate = (
        float(sampling_rate) if sampling_rate.strip() else default_sampling_rate
    )
...
def SinusoidDefaultParameters():
    ...
    duration = input(
        f"Enter the duration of the Sine (in seconds - Press Enter to use default value = {default_duration}): "
    )
    duration = float(duration) if duration.strip() else default_duration

    default_sampling_rate = 128e3
    sampling_rate = input(
        f"Enter the sampling rate of the Sine (in Hz - Press Enter to use default value = {default_sampling_rate}): "
    )
    sampling_rate = (
        float(sampling_rate) if sampling_rate.strip() else default_sampling_rate
    )
...
def RayleighDistance(a, f0, c1, r):
    ...
    print(
        f"Slant range between monostatic sonar and surface of the target: {r - a:.3f} m"
    )
...
def GetFormFunctionAbove(f1, f2, gamma_11, gamma_11_squared, k, a, theta_i):
    ...
    formfunction = (
        term1 + term2 + term3
    )  # * np.exp(-1j * k * r)  # Equation 30 from paper 2012
...
def GetEchoAbove(
    f1, f2, gamma_11, k, a, theta_i, gamma_11_squared, S_f, fft_points, flag
):
    term1 = f1
    term2 = 2 * gamma_11 * f2 * np.exp(2j * k * a * (np.sin(theta_i)))
    term3 = gamma_11_squared * f1 * np.exp(4j * k * a * np.sin(theta_i))
    ...
    f_conj_above = np.conjugate(
        f_above[1:]
    )  # Elimina el primer elemento de f, que es la frecuencia cero,
    ...
    echo_above = ifft(
        echo_spectrum_above, fft_points
    )  # Para tratar de replicar el de Tesei
...
def ModesSolid(n, k1, freqs, x, x1, x2, theta, flag, rho1, rho2, r, field="near-field"):
    ...
    for i in range(len(freqs)):
        ...
        d12 = ((2 * n * (n + 1) - (x2[i] ** 2)) * spherical_jn(n, x1[i])) - (
            4 * x1[i] * spherical_jn(n, x1[i], True)
        )  # original
        d13 = (
            2
            * n
            * (n + 1)
            * ((x2[i] * spherical_jn(n, x2[i], True)) - spherical_jn(n, x2[i]))
        )  # original
        ...
        d32 = 2 * (
            spherical_jn(n, x1[i]) - (x1[i] * spherical_jn(n, x1[i], True))
        )  # original
        d33 = 2 * x2[i] * spherical_jn(n, x2[i], True) + (
...
def ModesVacuum(
    n, k1, freqs, x, xs, xL, ys, yL, theta, flag, rho_tilde, r, field="near-field"
):
    fn_array = np.zeros(freqs.size, dtype=complex)
    ...
    for i in range(len(freqs)):
        ...
        d12 = ((2 * n * (n + 1) - (xs[i] ** 2)) * jn_x_L_freq) - (
            4 * xL[i] * jn_x_L_freq_deriv
        )
        d13 = 2 * n * (n + 1) * (xs[i] * jn_x_s_freq_deriv - jn_x_s_freq)
        d14 = (2 * n * (n + 1) - (xs[i] ** 2)) * yn_x_L_freq - (
            4 * xL[i] * yn_x_L_freq_deriv
        )
        ...
        d33 = (2 * xs[i] * jn_x_s_freq_deriv) + (
            ((xs[i] ** 2) - 2 * n * (n + 1) + 2) * jn_x_s_freq
        )
        d34 = 2 * (yn_x_L_freq - (xL[i] * yn_x_L_freq_deriv))
        d35 = (2 * xs[i] * yn_x_s_freq_deriv) + (
            ((xs[i] ** 2) - 2 * n * (n + 1) + 2) * yn_x_s_freq
        )
        d42 = ((2 * n * (n + 1) - (ys[i] ** 2)) * jn_y_L_freq) - (
            4 * yL[i] * jn_y_L_freq_deriv
        )
        d43 = 2 * n * (n + 1) * (ys[i] * jn_y_s_freq_deriv - jn_y_s_freq)
        d44 = ((2 * n * (n + 1) - ys[i] ** 2) * yn_y_L_freq) - (
            4 * yL[i] * yn_y_L_freq_deriv
        )
        d45 = 2 * n * (n + 1) * (ys[i] * yn_y_s_freq_deriv - yn_y_s_freq)
        d52 = 2 * (jn_y_L_freq - yL[i] * jn_y_L_freq_deriv)
        d53 = (2 * ys[i] * jn_y_s_freq_deriv) + (
            (ys[i] ** 2) - 2 * n * (n + 1) + 2
        ) * jn_y_s_freq
        d54 = 2 * (yn_y_L_freq - yL[i] * yn_y_L_freq_deriv)
        d55 = (2 * ys[i] * yn_y_s_freq_deriv) + (
            (ys[i] ** 2) - 2 * n * (n + 1) + 2
        ) * yn_y_s_freq
...
def ModesFluid(
    ...
    for i in range(len(freqs)):
        ...
        d12 = (2 * n * (n + 1) - k_s2[i] ** 2 * a**2) * spherical_jn(
            n, k_d2[i] * a
        ) - 4 * k_d2[i] * a * spherical_jn(n, k_d2[i] * a, True)
        d13 = (2 * n * (n + 1) - k_s2[i] ** 2 * a**2) * spherical_yn(
            n, k_d2[i] * a
        ) - 4 * k_d2[i] * a * spherical_yn(n, k_d2[i] * a, True)
        d14 = (
            2
            * n
            * (n + 1)
            * (
                k_s2[i] * a * spherical_jn(n, k_s2[i] * a, True)
                - spherical_jn(n, k_s2[i] * a)
            )
        )
        d15 = (
            2
            ...
            * (n + 1)
            * (
                k_s2[i] * a * spherical_yn(n, k_s2[i] * a, True)
                - spherical_yn(n, k_s2[i] * a)
            )
        )
        ...
        d32 = 2 * (
            spherical_jn(n, k_d2[i] * a)
            - k_d2[i] * a * spherical_jn(n, k_d2[i] * a, True)
        )
        d33 = 2 * (
            spherical_yn(n, k_d2[i] * a)
            - k_d2[i] * a * spherical_yn(n, k_d2[i] * a, True)
        )
        d34 = 2 * k_s2[i] * a * spherical_jn(n, k_s2[i] * a, True) + (
            k_s2[i] ** 2 * a**2 - 2 * n * (n + 1) + 2
        ) * spherical_jn(n, k_s2[i] * a)
        d35 = 2 * k_s2[i] * a * spherical_yn(n, k_s2[i] * a, True) + (
            k_s2[i] ** 2 * a**2 - 2 * n * (n + 1) + 2
        ) * spherical_yn(n, k_s2[i] * a)

        d42 = -4 * k_d2[i] * b * spherical_jn(n, k_d2[i] * b, True) + (
            2 * n * (n + 1) - k_s2[i] ** 2 * b**2
        ) * spherical_jn(n, k_d2[i] * b)
        d43 = -4 * k_d2[i] * b * spherical_yn(n, k_d2[i] * b, True) + (
            2 * n * (n + 1) - k_s2[i] ** 2 * b**2
        ) * spherical_yn(n, k_d2[i] * b)
        ...
        d44 = (
            ...
            * (n + 1)
            * (
                k_s2[i] * b * spherical_jn(n, k_s2[i] * b, True)
                - spherical_jn(n, k_s2[i] * b)
            )
        )
        d45 = (
            2
            ...
            * (n + 1)
            * (
                k_s2[i] * b * spherical_yn(n, k_s2[i] * b, True)
                - spherical_yn(n, k_s2[i] * b)
            )
        )
        ...
        d62 = 2 * (
            spherical_jn(n, k_d2[i] * b)
            - k_d2[i] * b * spherical_jn(n, k_d2[i] * b, True)
        )
        d63 = 2 * (
            spherical_yn(n, k_d2[i] * b)
            - k_d2[i] * b * spherical_yn(n, k_d2[i] * b, True)
        )
        d64 = 2 * k_s2[i] * b * spherical_jn(n, k_s2[i] * b, True) + (
            k_s2[i] ** 2 * b**2 - 2 * n * (n + 1) + 2
        ) * spherical_jn(n, k_s2[i] * b)
        d65 = 2 * k_s2[i] * b * spherical_yn(n, k_s2[i] * b, True) + (
            k_s2[i] ** 2 * b**2 - 2 * n * (n + 1) + 2
        ) * spherical_yn(n, k_s2[i] * b)
