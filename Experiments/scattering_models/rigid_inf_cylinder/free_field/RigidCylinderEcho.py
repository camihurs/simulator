...
def fn_rigid_complex(n, x):
    """
    Amplitud compleja de f_n^(r)(πx) para un cilindro rígido según ecuación (19)
    """
    # Factor de Neumann
    epsilon_n = 1 if n == 0 else 2

    # Derivadas usando funciones específicas
    J_n_prime = jvp(n, x, 1)
    H_n_prime = hankel1p(n, x)

    # Calcular la función compleja según la ecuación (19)
    fn_complex = -2 * ((-1)**n) * epsilon_n * J_n_prime / ((np.sqrt(1j * np.pi * x)) * H_n_prime)

    return fn_complex



def rigid_cylinder_form_function(x, n_max=20):
    """
    Calcular la forma función total del cilindro rígido sumando los modos hasta n_max
    Esta función representa la forma función total o sección transversal normalizada
    """
    # Inicializar con ceros complejos
    f_total = np.zeros(len(x), dtype=complex)

    # Sumar todas las contribuciones de los modos
    for n in range(n_max + 1):
        fn = fn_rigid_complex(n, x)
        f_total += fn
...
def PlotEcho(echo_normalized, sampling_rate, source, field, params, target, condition, check = False):
    ...
    if choice == '1':
        ...
        specular_echo  = (r - params['a'])/params['c1']
        back_face_echo = (r + params['a'])/params['c1']
        plt.figure(figsize=(12, 6))
        ...
        plt.tick_params(axis='both', which='major', labelsize=18)
        plt.grid()
        plt.axvline(x=(r-params['a'])*1000/params['c1'], color='red', linestyle='--', label=f'Specular echo = {specular_echo:5f} s')
        #plt.axvline(x=(r+params['a'])*1000/params['c1'], color='red', linestyle='--', label=f'Specular echo = {specular_echo:5f} s')
        plt.legend()
        plt.show()
        plt.clf()
...
k1a = 2 * np.pi * a * freqs_positive / c
print(f"\nThe minimum k1a is: {k1a[0]}")
print(f"The maximum k1a is: {k1a[-1]}")
print("\nStep size in ka:", k1a[1] - k1a[0])

print("\nComputing the echo for the rigid cylinder in free field...")

target = 'RigidCylinder'
condition = 'FF'

# Calcular la forma función total para el cilindro rígido
f = rigid_cylinder_form_function(k1a, n_max=n_max_rigid)
total_form_function = f * np.exp(1j * k1a/a * r)
...
PlotFormFunction(total_form_function, k1a, "rigid cylinder", "free field (computed to get the echo)", params, 0, np.max(freqs_positive), len(freqs_positive), 0, np.max(k1a))


f_final_rigid = GetFormFunctionEcho(total_form_function)
...
if max_samples > len(echo_rigid_normalized):
    print(f"Advertencia: Solicitaste recortar a {max_time_ms} ms, pero la señal solo tiene {len(echo_rigid_normalized)/sampling_rate*1000:.2f} ms")
    max_samples = len(echo_rigid_normalized)

# Recortar el eco
echo_rigid_normalized_truncated = echo_rigid_normalized[:max_samples]
...
echo_spectrum = fft(echo_rigid_normalized, fft_points)
freqs = fftfreq(fft_points, d=1/sampling_rate)
...
info_text = (f'Source: {source}\n'
            ...
            f'Field: {field}\n'
            f'Surrounding fluid (Medium 1): {params["medium1"]}\n'
            f'Number of modes used (n_max): {n_max_rigid}')
plt.plot(filtered_freqs, np.abs(filtered_spectrum))
plt.text(0.98, 0.02, info_text,
                    transform=plt.gca().transAxes,
                    verticalalignment='bottom',
